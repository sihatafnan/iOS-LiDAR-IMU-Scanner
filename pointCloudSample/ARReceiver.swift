...
//        }
    ...
    //for point cloud
    func depthBufferToPoints(depthBuffer: CVPixelBuffer, intrinsics: simd_float3x3, transform: simd_float4x4) -> [simd_float3] {
        let width = CVPixelBufferGetWidth(depthBuffer)
        let height = CVPixelBufferGetHeight(depthBuffer)
//        print("Depth Map Resolution: \(width)x\(height)")
        ...
        for y in 0..<height {
            for x in 0..<width {
                let index = y * CVPixelBufferGetBytesPerRow(depthBuffer) / MemoryLayout<Float>.size + x
                let depth = baseAddress!.assumingMemoryBound(to: Float.self)[index]
                
                // Ignore invalid depth values
                guard depth > 0 else { continue }
                
                // Project (x, y, depth) to camera space
                let xNorm = (Float(x) - intrinsics[0][2]) / intrinsics[0][0]
                let yNorm = (Float(y) - intrinsics[1][2]) / intrinsics[1][1]
                let pointCamera = simd_float3(xNorm * depth, yNorm * depth, depth)
                
                // Transform to world space
                let pointWorld = transform * simd_float4(pointCamera, 1.0)
                points.append(simd_float3(pointWorld.x, pointWorld.y, pointWorld.z))
            }
        }
    ...
    func savePointCloudToPLY(points: [simd_float3], to url: URL) {
        let header = """
        ply
        format ascii 1.0
        ...
        end_header
        """
        let pointCloudData = points.map { "\($0.x) \($0.y) \($0.z)" }.joined(separator: "\n")
        let plyData = "\(header)\n\(pointCloudData)"
        do {
            try plyData.write(to: url, atomically: true, encoding: .utf8)
            print("Point cloud saved to \(url.path)")
        } catch {
    ...
    func endRecord() {
        ...
        //point cloud
        var all_points: [simd_float3] = []
        for points in self.pointClouds {
            for point in points {
                all_points.append(point)
            }
        }
        let pointCloudURL = FileManager.default
            .urls(for: .documentDirectory, in: .userDomainMask)
            .first!.appendingPathComponent(self.directory + "/point_cloud.ply")
        savePointCloudToPLY(points: all_points, to: pointCloudURL)
    ...
    func session(_ session: ARSession, didUpdate frame: ARFrame) {
        if(frame.sceneDepth != nil) && (frame.smoothedSceneDepth != nil) {
            ...
            if self.isRecord {
                ...
                if let depthimage = frame.sceneDepth?.depthMap{
                    let points = depthBufferToPoints(depthBuffer: depthimage,
